...
void main() {
  ...
  group('get season detail', () {
    ...
    test('should return tv detail when the response code is 200', () async {
      // arrange
      when(mockHttpClient
              .get(Uri.parse('$BASE_URL/tv/$tvId/season/$seasonNum?$API_KEY')))
          .thenAnswer((_) async =>
              http.Response(readJson('dummy_data/season_detail.json'), 200));
      // act
      final result =
          await dataSource.getSeasonDetail(tvId: tvId, seasonNum: seasonNum);
      // assert
    ...
    test('should throw Server Exception when the response code is 404 or other',
      ...
      when(mockHttpClient
          ...
          .thenAnswer((_) async => http.Response('Not Found', 404));
      // act
      final call = dataSource.getSeasonDetail(tvId: tvId, seasonNum: seasonNum);
      // assert
      expect(() => call, throwsA(isA<ServerException>()));
    });
  });
